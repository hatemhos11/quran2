...
function AzkarCard({
  ...
  return (
    <View
        ...
        <View style={styles.cardTop}>
          <Pressable
            ...
            style={[
              styles.counter,
              {
                backgroundColor: done ? `${c.accent}22` : `${c.accent}12`,
                borderColor: done ? c.accent : `${c.accent}44`,
              },
            ]}>
            <Text style={[styles.counterValue, { color: done ? c.accent : c.text }]}>
              {done ? ar.azkarCompleted : `${current}/${target}`}
            </Text>
            {!done ? (
              <Text style={[styles.counterHint, { color: c.textSecondary }]}>{ar.azkarTapToCount}</Text>
            ) : null}
          </Pressable>
          {current > 0 ? (
            <Pressable onPress={onReset} hitSlop={8}>
              <Text style={[styles.reset, { color: c.accentMuted }]}>{ar.azkarReset}</Text>
            </Pressable>
          ) : null}
        </View>
      ) : null}
      ...
      {item.description ? (
        <View style={[styles.section, { backgroundColor: `${c.accentMuted}10` }]}>
          <Text style={[styles.sectionLabel, { color: c.accentMuted }]}>{ar.azkarVirtue}</Text>
      ...
      {item.reference ? (
        <View style={styles.referenceRow}>
          <MaterialCommunityIcons name="book-open-variant" size={14} color={c.accent} />
          <Text style={[styles.reference, { color: c.textSecondary }]}>{item.reference}</Text>
        </View>
      ) : null}
    </View>
  );
...
export function AzkarCategoryScreen({ navigation, route }: Props) {
  ...
  const renderItem = useCallback(
    ({ item }: { item: AzkarItem }) => (
      <AzkarCard item={item} isDark={isDark} arabicFamily={arabicFamily} />
    ),
    [arabicFamily, isDark]
  );
  ...
  const listHeader = useMemo(
    () => (
      <View style={styles.listHeader}>
        <View style={[styles.pill, { backgroundColor: `${c.accent}18` }]}>
          <Text style={[styles.pillText, { color: c.accent }]}>{ar.azkarItems(items.length)}</Text>
        </View>
      </View>
    ),
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: c.background }]} edges={['top']}>
      <Appbar.Header style={[styles.appBar, { backgroundColor: c.surface }]}>
        <Appbar.BackAction onPress={() => navigation.goBack()} accessibilityLabel={ar.azkar} />
        <Appbar.Content
          title={category}
      ...
      {loading ? (
        ...
        <EmptyState isDark={isDark} title={ar.somethingWrong} message={error} />
      ) : (
        <FlatList
          data={items}
