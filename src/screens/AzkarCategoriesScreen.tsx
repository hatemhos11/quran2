...
import { sortAzkarCategories, usePinnedAzkarStore } from '@/store/pinnedAzkarStore';
...
export function AzkarCategoriesScreen({ navigation }: Props) {
  ...
  const pinnedCount = useMemo(
    () => sortedCategories.filter((c) => pinnedCategories.includes(c.name)).length,
  ...
  const cardBorder = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(44,62,80,0.08)';
  ...
  const renderItem = useCallback(
          ...
          <View
            style={[
              styles.card,
              {
                ...
                borderColor: pinned ? `${c.accent}55` : cardBorder,
              },
            ]}>
            <Pressable
              onPress={() => navigation.navigate('AzkarCategory', { category: item.name })}
              style={({ pressed }) => [
                styles.cardMain,
                {
              ...
              <View style={[styles.iconWrap, { backgroundColor: `${c.accent}18` }]}>
                <MaterialCommunityIcons name="hands-pray" size={22} color={c.accent} />
              </View>
              ...
              <View style={[styles.countBadge, { backgroundColor: `${c.accentMuted}22` }]}>
                <Text style={[styles.countText, { color: c.accentMuted }]}>{item.itemCount}</Text>
              </View>
              <MaterialCommunityIcons name="chevron-left" size={22} color={c.textSecondary} />
            </Pressable>
            <IconButton
              ...
              accessibilityLabel={pinned ? ar.unpinAzkarCategoryA11y : ar.pinAzkarCategoryA11y}
    ...
    [c, cardBorder, isPinned, navigation, pinnedCount, query, sortedCategories.length, togglePin]
  );

  const header = useMemo(
    () => (
      <View style={styles.header}>
        <Text style={[styles.title, { color: c.text }]}>{ar.azkar}</Text>
        <Text style={[styles.subtitle, { color: c.textSecondary }]}>
          {categories.length > 0 ? ar.azkarCategoriesCount(categories.length) : ' '}
        </Text>
        <Searchbar
  ...
  if (error && categories.length === 0) {
    return (
      <SafeAreaView style={[styles.root, { backgroundColor: c.background }]} edges={['top']}>
        {header}
        <EmptyState isDark={isDark} title={ar.somethingWrong} message={error} onRetry={() => load(query)} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: c.background }]} edges={['top']}>
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? (
            <Text style={[styles.empty, { color: c.textSecondary }]}>{ar.azkarNoResults}</Text>
          ) : null
        }
      />
    </SafeAreaView>
